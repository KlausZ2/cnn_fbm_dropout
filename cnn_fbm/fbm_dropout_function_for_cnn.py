...
class DropoutFBM(nn.Module):
    ...
    def forward(self, input: Tensor) -> Tensor:
        ...
        # Ensure the mask matches input dimensions for convolutional layers
        if self.is_conv:
            _, _, h, w = input.shape
            self.mask = torch.nn.functional.interpolate(self.mask.unsqueeze(0).unsqueeze(0), size=(h, w), mode='nearest').squeeze(0).squeeze(0)

        # Move the mask to the correct device and dtype
        if self.dtype:
