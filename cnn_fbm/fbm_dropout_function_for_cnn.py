...
class DropoutFBM(nn.Module):
    ...
    def forward(self, input: Tensor) -> Tensor:
        ...
        batch_size, num_features, h, w = input.shape

        # Generate independent masks for each feature map
        masks = []
        for _ in range(num_features):
            # Generate a new mask for the feature map
            mask = self.get_mask()
            # Resize the mask to match the height and width of the input
            mask = torch.nn.functional.interpolate(mask.unsqueeze(0).unsqueeze(0), size=(h, w), mode='nearest').squeeze(0).squeeze(0)
            masks.append(mask)

        # Stack masks along the feature dimension
        masks = torch.stack(masks, dim=0).to(self.device, self.dtype)

        # Expand masks to match the batch size
        masks = masks.unsqueeze(0).expand(batch_size, -1, -1, -1)

        # Update iterations and dropout rates
        self.current_iter += 1
        self.total_iter += 1
        self.update_dropout_rate()

        # Apply the mask to the input and scale the output
        return input * masks * (1 / (1 - self.now_dropout_rate))

    def get_mask(self):
        # Generate the mask using the is_touching function
    ...
    def get_grid(self):
        ...
        # Calculate size of each rectangle in the grid
        size_row = 1.0 / n_row
        size_col = 1.0 / n_col

        # Define gaps for neuron boundaries
        gap_y = size_row / 4
        gap_x = size_col / 4

        # Create the grid with boundaries for each neuron
        grid = {}
    ...
    def is_touching(self):
        # Helper function to check if an agent is within a grid cell
        ...
        # Initialize the mask with all neurons active
        is_touching = torch.ones(self.grid_size)

        # Calculate current iteration start point for the agents
        t = self.current_iter * self.sliding_window_len
        curr_agents_x = [agent_x[t:t + self.n_samples] for agent_x in self.agents_x]
        curr_agents_y = [agent_y[t:t + self.n_samples] for agent_y in self.agents_y]

        # Check each grid cell for interaction with FBM agents
        for i in range(self.n_col):
            for j in range(self.n_row):
                for x, y in zip(curr_agents_x, curr_agents_y):
                    if is_in(x, y, self.grid[(i, j)]):
                        is_touching[(i, j)] = 0

        # Rotate the mask to align with input format
        return torch.rot90(is_touching)
